CLASS zcl_data_gen_rap_ag DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_rap_ag IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.

     out->write( 'Adding Travel data' ).

    DELETE FROM ztb_travel_ag_a.

    INSERT ztb_travel_ag_a FROM (
    ...
    DELETE FROM ztb_booking_ag_a.

    INSERT ztb_booking_ag_a FROM (

        SELECT
          FROM /dmo/booking
          JOIN ztb_travel_ag_a ON /dmo/booking~travel_id = ztb_travel_ag_a~travel_id
          JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
          FIELDS  "client,
                  uuid( ) AS booking_uuid,
                  ztb_travel_ag_a~travel_uuid AS parent_uuid,
                  ...
                  ztb_travel_ag_a~last_changed_at AS local_last_changed_at ).


    DELETE FROM ztb_bksppl_ag_a.

    out->write( 'Adding Booking Supplements data' ).

    INSERT ztb_bksppl_ag_a FROM (
       SELECT FROM /dmo/book_suppl AS supp
              JOIN ztb_travel_ag_a  AS trvl ON trvl~travel_id = supp~travel_id
              JOIN ztb_booking_ag_a AS book ON book~parent_uuid = trvl~travel_uuid
