CLASS zcl_data_gen_ac_paris DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_ac_paris IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    DATA: lt_acc_categ TYPE TABLE OF ztb_acc_categ_mp,
          lt_catego    TYPE TABLE OF ztb_catego_map,
          lt_clientes  TYPE TABLE OF ztb_clientes_map,
          lt_clnts_lib TYPE TABLE OF ztb_clnts_lib_mp,
          lt_libros    TYPE TABLE OF ztb_libros_map.
   ...
   ( bi_categ = 'H' tipo_acceso = '4' ) ).
    ...
    DELETE FROM ztb_acc_categ_mp.
    INSERT ztb_acc_categ_mp FROM TABLE @lt_acc_categ.
...
******** ZTB_CATEGO ********
    ...
    "Delete possible entries; insert new entries
    DELETE FROM ztb_catego_map.
    INSERT ztb_catego_map FROM TABLE @lt_catego.
    IF sy-subrc EQ 0.
...
******** ZTB_CLIENTES ********
    ...
    "Delete possible entries; insert new entries
    DELETE FROM ztb_clientes_map.
    INSERT ztb_clientes_map FROM TABLE @lt_clientes.
    IF sy-subrc EQ 0.
...
( id_cliente = '540867547H' id_libro = '000020' ) ).
    "Delete possible entries; insert new entries
    DELETE FROM ztb_clnts_lib_mp.
    INSERT ztb_clnts_lib_mp FROM TABLE @lt_clnts_lib.
    IF sy-subrc EQ 0.
      out->write( |Clientes-Libros: { sy-dbcnt } registros insertados| ).
    ENDIF.
...
"Delete possible entries; insert new entries
DELETE FROM ztb_libros_map.
INSERT ztb_libros_map FROM TABLE @lt_libros.
 IF sy-subrc EQ 0.
